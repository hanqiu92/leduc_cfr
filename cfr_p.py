...
def get_policy_from_regret_CFR_plus(p,I,A,regret_info):
    ## regret matching
    Na = len(A)
    if p == 0:
...
def CFR_plus(h,players,p_curr,prob_info,regret_info,avg_policy_info,count_info):
    count_info['count'] += 1
    if h.is_terminated():
    ...
    ## recursive search
    probs = get_policy_from_regret_CFR_plus(p,I,A,regret_info)
    values = {}
    value = 0
    for a in A:
        prob_a = probs[a]
        ...
        value_a = CFR_plus(h_new,players,p_curr,prob_info_new,
...
def discount_avg_policy_CFR_plus(avg_policy_info,iter_=1):
    discount = iter_ / (iter_ + 1)
    for policy in avg_policy_info.values():
        for a in policy.keys():
            policy[a] *= discount

def train_CFR_plus(h0,players,ps,regret_info,avg_policy_info,count_info,iter_=0,**kwargs):
    if iter_ > 0:
        discount_avg_policy_CFR_plus(avg_policy_info,iter_=iter_)
    for p_curr in ps:
        prob_info = {'all':1,'i':np.ones((len(players),)),'-i':np.ones((len(players),))}
        CFR_plus(h0.copy(),players,p_curr,prob_info,regret_info,avg_policy_info,count_info)
    for regret in regret_info.values():
        for a in regret.keys():
            regret[a] = max(regret[a],0)
