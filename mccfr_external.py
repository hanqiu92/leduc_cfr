...
def get_policy_from_regret_MCCFR_external(p,I,A,regret_info):
    ## regret matching
    Na = len(A)
    if p == 0:
...
def MCCFR_external(h,players,p_curr,prob_info,regret_info,avg_policy_info,count_info):
    count_info['count'] += 1
    if h.is_terminated():
    ...
    probs = get_policy_from_regret_MCCFR_external(p,I,A,regret_info)
    if p == p_curr:
        ...
        for a in A:
            ...
            h_new = h.copy()
            h_new.transition(a)
            value_a = MCCFR_external(h_new,players,p_curr,prob_info,
        ...
        h_new = h.copy()
        h_new.transition(a)
        value = MCCFR_external(h_new,players,p_curr,prob_info,
                    regret_info,avg_policy_info,count_info) ## v(h,a)
...
def train_MCCFR_external(h0,players,ps,regret_info,avg_policy_info,count_info,**kwargs):
    for p_curr in ps:
        prob_info = {'all':1,'i':np.ones((len(players),)),'-i':np.ones((len(players),))}
        MCCFR_external(h0.copy(),players,p_curr,prob_info,regret_info,avg_policy_info,count_info)

def discount_avg_policy_MCCFR_external(avg_policy_info,iter_=1):
    discount = iter_ / (iter_ + 1)
    for policy in avg_policy_info.values():
        for a in policy.keys():
            policy[a] *= discount

def train_linear_MCCFR_external(h0,players,ps,regret_info,avg_policy_info,count_info,iter_=0,**kwargs):
    if iter_ > 0:
        discount_avg_policy_MCCFR_external(avg_policy_info,iter_=iter_)
    for p_curr in ps:
        prob_info = {'all':1,'i':np.ones((len(players),)),'-i':np.ones((len(players),))}
        MCCFR_external(h0.copy(),players,p_curr,prob_info,regret_info,avg_policy_info,count_info)
